...
def _prime_board(pb_state=None):
    ...
    st.caption("**Few trades, the ones that win.** Elite score-80+ fire "
               "(MAX excluded — measured worse) + mid-band volatility "
               "(ATR 40–80) + calm 6h, entered on the 30m confirmation, "
               "**banked at TP1** (riding tested worse). Mined from 134 entries over 108 days: **74.5% "
    ...
    for _i, _r in enumerate(_cards[:4]):
        ...
        _c1.markdown(
            ...
            f"m ago · {_r.get('tier') or 'HIGH'} "
            f"{float(_r.get('score') or 0):.0f}"
            f"{' · ⭐ TOP BAND' if float(_r.get('score') or 0) >= 85 else ''}"
            f" · calm 6h "
...
if active_section == "🎮 Demo $1,500":
    ...
    st.markdown(
        ...
        "clip:text;-webkit-text-fill-color:transparent;background-"
        "clip:text'>🎮 DEMO $1,500 — GEN 7</span><br>"
        "<span style='color:#9aa7c7;font-size:0.82rem'>Fresh ledger "
        "from 2026-08-26 (user: rerun from today). SEAT MAP — 💥⚡ "
        "STRONG TRIGGERS + 🔄 RE-RUNs share 5 seats · 🎯 "
        "BEST-OF-BEST gets 3 (💎 BEST ZONE cards carrying the "
        "telegram confidence read ≥80 — the 98+ ones outrank "
        "everything in the lane) · 💎 ELITE FAMILY gets 2 (elite "
        "cream + 💎✅ confirmed/re-entry, max scores). Slots are a "
        "CEILING, not a quota — empty beats mediocre. 🔄 ROTATION: "
        "when a stronger signal is waiting and its seats are full, "
        "a position whose SIGNAL HAS DIED is rotated out — banked "
        "if positive, cut if negative; a losing position stands "
    ...
    for _p in _openp:
        ...
        st.markdown(
            ...
            f"${_p.get('notional', 0):,.0f} (~{_p.get('lev', '?')}× "
            f"the account)"
...
if active_section == "🧪 Paper Trader":
    ...
    with right_col:
        ...
        # ================================================================
        # 🎯 QUALITY GATE — DIRECTION-BLIND (2026-06-06 v3)
        #
        # USER: 'I dont want to see more short picks. I want to pick the
        # best trades to be doesnt matter its long or short'.
        #
        # Pure quality filter — no direction bias, no regime tilt at the
        # gate level. The regime detector already tilts the SCORE itself
        # (via _apply_regime_tilt in experimental_signals.py) so aligned
        # picks score higher naturally. The gate just enforces a minimum
        # quality bar that works equally for LONG and SHORT.
        #
        # RULE: a pick surfaces on the openable TOP CONVICTION board if
        # ANY of:
        #   • 2+ independent systems agree (CONVERGENCE / SURE SHOT /
        #     ELITE side-match / forecast aligned 3/3)
        #   • combined score >= 85 (genuinely strong single-source)
        #
        # WHY THIS BAR:
        #   Audited closed-trade history, both periods:
        #     - Winners almost always had 2+ system confirmation OR a
        #       very high score (88+).
        #     - Losers were typically alerts-engine-only at score 73-85
        #       with nothing else corroborating.
        #   This gate filters the lonely-signal losers and lets the
        #   confirmed picks through — equally on either side.
        # ================================================================
        _quality_blocked = []
        _kept_picks = []
        # Make elite_lookup safely available (may not exist if ELITE
        # block hasn't run yet on this turn — guard against NameError)
        try:
            _ql_elite_lookup = _elite_lookup
        except NameError:
            _ql_elite_lookup = {}
        for _pk in _bot_picks:
            ...
            if _pk_elite and (_pk_elite.get("side") or "").upper() == _pk_side:
                _qg_count += 1
            if ("forecast confirms" in _pk_fc_label
                    and "aligned 3/3" in _pk_fc_label):
                _qg_count += 1
            # DIRECTION-BLIND quality bar
            _keep = (_qg_count >= 2 or _pk_combined >= 85)
            if _keep:
                _kept_picks.append(_pk)
            else:
                _quality_blocked.append(
        ...
        # ================================================================
        # 🎯 QUALITY FILTER BANNER — shows when picks were dropped for
        # failing the regime-aware quality bar. NOT a direction block —
        # both LONG and SHORT can pass; the bar just adapts to regime.
        # ================================================================
        if _quality_blocked:
            _blocked_n = len(_quality_blocked)
            _blocked_disp = ", ".join(
                f"{sym.replace('USDT','')} {side}({sc})"
                for sym, side, sc, _ in _quality_blocked[:5])
            _more = (f" +{_blocked_n - 5} more"
                     if _blocked_n > 5 else "")
            st.markdown(
                f"<div style='background:linear-gradient(135deg,"
                f"rgba(91,142,255,0.12),rgba(139,92,246,0.08));"
                f"border:1px solid rgba(91,142,255,0.40);"
                ...
                f"{_blocked_n} lonely-signal pick"
                f"{'s' if _blocked_n != 1 else ''} hidden "
                f"(direction-blind)"
                f"</span></div>"
                f"<div style='color:#cfd2d8;font-size:0.82rem;"
                f"line-height:1.55'>"
                f"<b>Hidden:</b> {_blocked_disp}{_more}<br/>"
                f"<span style='color:#aab;font-size:0.76rem'>"
                f"Bar: 2+ independent systems agree OR score ≥85. "
                f"Equal rule for LONG and SHORT."
                f"</span></div></div>",
                unsafe_allow_html=True)
        ...
        if _u_scan_fn is not None:
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            st.caption(
                "**Multi-system confirmation board.** Picks where 2+ "
                "signal lanes independently agree on the same coin + "
                "side at STRONG+ (score ≥80). **Lanes:** 🎯 Pattern "
                "Scout · 🔭 Reversal Approach · 🔄 V-Bottom Recovery · "
                "⚡ Early Momentum (CVD+TTM+SMC) · 💱 Derivatives "
                "Velocity (funding+OI) · 🔁 Rebound Radar · 🚀 "
...
if active_section == "🤖 24/7 Agent":
    ...
    def _strength_bar(score: float) -> str:
        """ASCII strength bar — used in the collapsed title for quick read."""
        try:
    ...
    def _render_deep_view(symbol: str, base: str, side: str,
        ...
        for tf in ("15m", "1h", "4h", "1d"):
            r = (per_tf or {}).get(tf) or {}
            tf_side = str(r.get("side") or "—").upper()
        ...
        # --- 3. Drivers / candle formation — pull from the strongest TF
        st.markdown("#### 🕯️ Candle formation & top drivers")
        ...
        if forming:
            st.markdown(
                _render_chip(
                    "🔭 FORECAST FORMING — leading signals diverge "
                    "from current trend",
                    "rgba(245,166,35,0.18)", "#f5a623"),
                unsafe_allow_html=True)

        # --- 4. Trade plan — full detail
        if plan and _has_plan(plan):
            ...
            st.markdown(
                ...
                f"- **TP1:** `{f['tp1']:g}` (`{tp1_pct:+.2f}%`) — "
                f"R:R **{f['rr']:.2f}**"
                + (f"  \n- **TP2:** `{f['tp2']:g}` (`{tp2_pct:+.2f}%`) — "
                   f"R:R **{rr2:.2f}**"
                   if f["tp2"] > 0 else ""))
        else:
    ...
    def _agent_section():
        ...
        if not reports:
            ...
            for rep in reports:
                ...
                with st.expander(title, expanded=False):
                    if unavailable:
                        st.warning(
                            f"⚠️ {symbol} unavailable on Binance right "
                            f"now — deep analysis skipped.")
                        continue
                    _render_deep_view(
        ...
        st.caption(
            ...
            "outside your portfolio. Conviction ≥**80** floor. Empty if "
        ...
        if not picks:
            st.info(
                ...
                "conviction ≥80 setups outside your portfolio.")
        else:
            for pk in picks[:10]:
...
if active_section == "💸 Live Trading":
    ...
    # --- 🎮→💸 GEN 10 LIVE EXECUTOR BOARD (user 2026-09-07: "the ui of
    # live trading should be similar to demo trading") — the demo
    # cockpit's visual language on the real-money ledger: gradient
    # header, equity/start/win-rate/seat tiles, equity curve, styled
    # seat cards. Read-only mirror of .live_exec.json — the WORKER
    # trades, this board only reports. Fail-soft everywhere.
    try:
        import live_executor as _lxm
        _lx_on = (os.environ.get("LIVE_EXECUTOR", "").strip().lower()
                  in ("1", "true", "yes", "on"))
        _g10_on = (os.environ.get("LIVE_GEN10", "").strip().lower()
                   in ("1", "true", "yes", "on"))
        _lx_hold = bool(getattr(_lxm, "GEN10_ENTRY_HOLD", False))
        ...
        if _lx_on or _g10_on or _lx_open or _lx_closed:
            ...
            _lx_st0 = float(_lx_s.get("starting_balance") or 0)
            _lx_net = _lx_bal - _lx_st0
            _lx_wins = sum(1 for c in _lx_closed
                           if float(c.get("pnl_usd") or 0) > 0)
            _lx_wr = (_lx_wins / len(_lx_closed) * 100
                      if _lx_closed else 0.0)
            _lx_days = max(0.0, (time.time()
                                 - float(_lx_s.get("started_at")
                                         or time.time())) / 86400)
            st.markdown(
                "<div style='background:linear-gradient(135deg,"
                "rgba(46,212,122,0.16),rgba(255,196,64,0.08));"
                "border:1.5px solid rgba(46,212,122,0.5);"
                "border-radius:16px;padding:16px 22px;margin:6px 0 "
                "12px'>"
                "<span style='font-size:1.5rem;font-weight:900;"
                "background:linear-gradient(90deg,#2ed47a,#ffc440);"
                "-webkit-background-clip:text;-webkit-text-fill-color:"
                "transparent;background-clip:text'>🎮→💸 GEN 10 LIVE "
                "— REAL MONEY</span><br>"
                "<span style='color:#9aa7c7;font-size:0.82rem'>The "
                "demo brain on your Bybit account: SAME seven "
                "streams, SAME conf-band gates (💥 strong trigger "
                "40-54 + 65-84 · 🚀 moonshot + 🎯 sniper 55-64 · ⚡ "
                "waking 40-54 · 🟢 confirm 40-54 + 65-84 · 🤝 DUO "
                "85+ · 💥🔮 trig×KR 40-54 + 65-84) · 8 seats, no "
                "rotation, margin = equity÷8 levered 10x (triggers/"
                "sniper/moonshot) / 8x (duo/trig×KR) / 6x (my-watch "
                "lanes) · 💰 BANK 100% AT TP1 or SL, nothing else — "
                "no BE, no trail · 72h time-stop · SL/TP parked ON "
                "the exchange · daily -10% halt · -15% kill switch. "
                "Every open/close buzzes 💸 on Telegram. REAL ORDERS "
                f"— day {_lx_days:.1f}.</span></div>",
                unsafe_allow_html=True)
            if _lx_s.get("halted"):
                ...
                st.info(
                    "⏳ Not armed yet — needs LIVE_EXECUTOR=1, "
                    "LIVE_GEN10=1, BYBIT_API_KEY/SECRET and "
                    "BYBIT_TESTNET=false in the Render environment, "
                    "and a server region Bybit accepts. It arms on "
                    "the first cycle that reads non-zero Unified "
                    "Trading equity, and buzzes 💸 ARMED.")
            elif _lx_hold:
                st.warning("⏸ ENTRY HOLD — armed, but opening "
                           "NOTHING until you give the word. Equity "
                           "sync, watchdog and position management "
                           "keep running.")
            _t1x, _t2x, _t3x, _t4x = st.columns(4)
            _t1x.metric("💼 Bybit equity", f"${_lx_bal:,.2f}",
                        (f"{_lx_net:+,.2f} "
                         f"({_lx_net / _lx_st0 * 100:+.1f}%)"
                         if _lx_st0 > 0 else None))
            _t2x.metric("🏁 Started with",
                        f"${_lx_st0:,.2f}" if _lx_st0 else "—")
            _t3x.metric("🏆 Win rate",
                        f"{_lx_wr:.0f}%" if _lx_closed else "—",
                        f"{_lx_wins}/{len(_lx_closed)} closed")
            _t4x.metric("📂 Seats", f"{len(_lx_open)}/8")
            _lx_hist = _lx_s.get("equity_hist") or []
            if len(_lx_hist) >= 2:
                _lx_hdf = pd.DataFrame(_lx_hist,
                                       columns=["ts", "equity"])
                _lx_hdf["time"] = pd.to_datetime(_lx_hdf["ts"],
                                                 unit="s")
                st.line_chart(_lx_hdf.set_index("time")["equity"],
                              height=220)
            elif _lx_s.get("started_at"):
                st.caption("📈 Equity curve appears after the first "
                           "worker cycles record snapshots (~5 min "
                           "each).")
            st.markdown("#### 📂 Open live positions")
            if not _lx_open:
                st.caption("· No seats filled"
                           + (" — entry HOLD is on." if _lx_hold else
                              " — the executor only takes in-zone "
                              "fires inside their conf bands. 💸 "
                              "buzzes land the moment it acts."))
            for _p in _lx_open:
                _lp = float(prices.get(_p.get("symbol")) or 0)
                _upnl = ((_lp - float(_p.get("entry") or 0))
                         * float(_p.get("qty") or 0)
                         * (1 if _p.get("side") == "LONG" else -1)
                         ) if _lp else 0.0
                _ucol = "#2ed47a" if _upnl >= 0 else "#ff5c5c"
                _age_h = (time.time()
                          - float(_p.get("opened_at") or 0)) / 3600
                _tagx = ("🎮 " + str(_p.get("src"))
                         if _p.get("gen10")
                         else str(_p.get("tier") or "manual/old"))
                st.markdown(
                    f"<div style='background:rgba(46,212,122,0.05);"
                    f"border:1.5px solid rgba(46,212,122,0.4);"
                    f"border-radius:12px;padding:10px 15px;"
                    f"margin:5px 0'>"
                    f"<b style='font-size:1.05rem'>{_p.get('base')}"
                    f"</b> <span style='color:"
                    f"{'#2ed47a' if _p.get('side') == 'LONG' else '#ff5c5c'};"
                    f"font-weight:800'>{_p.get('side')}</span> "
                    f"<span style='color:#8b93a7;font-size:0.72rem'>"
                    f"· {_tagx} · {_age_h:.1f}h · "
                    f"{_p.get('leverage')}x · margin "
                    f"${float(_p.get('margin') or 0):,.2f} · notional "
                    f"${float(_p.get('notional') or 0):,.2f}</span>"
                    f"<br><span style='color:#e6e9f0;font-size:"
                    f"0.85rem'>entry {float(_p.get('entry') or 0):g} "
                    f"· live {_lp:g} · SL "
                    f"{float(_p.get('stop') or 0):g} · TP1 "
                    f"{float(_p.get('tp1') or _p.get('target') or 0):g}"
                    f" · <b style='color:{_ucol}'>uP&L "
                    f"${_upnl:+,.2f}</b></span></div>",
                    unsafe_allow_html=True)
            if _lx_closed:
                with st.expander(
            ...
            # 🔌 connection probe (the $0.00 lesson): show Bybit's RAW
            # answer or error — never a silent zero.
            try:
                _pb = lb.client().get_wallet_balance(
                    accountType="UNIFIED", coin="USDT")
                _pb_rows = ((_pb.get("result") or {}).get("list")
                            or [])
                if _pb_rows:
                    _pb_eq = float(_pb_rows[0].get("totalEquity") or 0)
                    st.caption(f"🔌 Bybit answers: UNIFIED "
                               f"totalEquity = ${_pb_eq:,.2f}")
                else:
                    st.caption("🔌 Bybit answered but the UNIFIED "
                               "account list is EMPTY — this account "
                               "may not be upgraded to Unified "
                               "Trading, or the funds sit in another "
                               "wallet/subaccount.")
            except Exception as _pb_exc:
                st.caption(f"🔌 Bybit API error: {_pb_exc}")
            st.divider()
...
""")
    ...
    with lt_right:
        # 🎮→💸 GEN 10 (user 2026-09-07: 'remove this we are
        # following gen 10'): the SST1 Sure Shots board and the
        # Bot top-picks board are RETIRED from this section — the
        # GEN 10 executor at the top of the page is the only live
        # brain now. Revert: git show 4f4284d:app.py.
        st.markdown('### 🎮→💸 GEN 10 is the live brain')
        st.caption(
            'The manual signal boards (SST1 Sure Shots, bot top '
            'picks) are retired on your order — every live entry '
            'now comes from the GEN 10 executor: the same seven demo '
            'streams in their measured conf bands, mirrored at the '
            'top of this page. The manual open form on the left '
            'stays for emergencies; positions and history are '
            'below.')
    st.divider()
