...
if active_section == "🎮 Demo $1,500":
    ...
    st.markdown(
        ...
        "re-qualified elite) on TOP priority with at least 6-7 of "
        "the 10 seats · the 💎 ELITE FAMILY holds 4 seats split 2+2 "
        "— 💎✅ CONFIRMED ENTRIES (the validated pullback+confirm "
        "entry, the higher-weighted construct) get 2, raw elite "
        "conviction cream (🚀 approved + MAX grade / 90+ score / 2+ "
        "lanes at 85+) gets 2. Nothing else spends a cent. "
        "10 slots · "
        "one position per coin · WILD sizing: each slot margins "
        "balance÷10 and levers 5x (elite) / 7x (trigger "
        "and re-run) / 10x (🔥 burst ≥85, the validated A-grade) — "
        "swings of ~$25-100 per trade, losses included — thinner "
        "per trade than the 6-slot build (the pot splits 10 ways) "
        "but nearly double the account at work on a full board. "
        "Taker fees "
